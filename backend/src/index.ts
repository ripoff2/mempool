...
class Server {
	...
	async startServer(worker = false): Promise<void> {
		...
		if (config.BISQ.ENABLED) {
			...
			bisq.setPriceCallbackFunction(price =>
				websocketHandler.setExtraInitProperties('bsq-price', price),
			)
		...
		this.server.listen(
			config.MEMPOOL.HTTP_PORT,
			config.MEMPOOL.HTTP_HOST,
			() => {
				if (worker) {
					logger.info(`Mempool Server worker #${process.pid} started`)
				} else {
					logger.notice(
						`Mempool Server is running on ${config.MEMPOOL.HTTP_HOST},${config.MEMPOOL.HTTP_PORT}`,
					)
				}
			},
		)
	...
	async runMainUpdateLoop(): Promise<void> {
		try {
			...
			this.currentBackendRetryInterval = Math.min(
				this.currentBackendRetryInterval,
				60,
			)
		}
	}
	...
	healthCheck(): void {
		...
		if (!this.warnedHeapCritical && this.maxHeapSize > warnThreshold) {
			...
			logger.warn(
				`Used ${(this.maxHeapSize / stats.heap_size_limit).toFixed(
					2,
