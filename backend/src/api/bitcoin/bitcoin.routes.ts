...
class BitcoinRoutes {
	public initRoutes(app: Application) {
		app.get(
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'outspends',
				this.$getBatchedOutspends,
			)
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'cpfp/:txId',
				this.$getCpfpInfo,
			)
			.get(
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'backend-info',
				this.getBackendInfo,
			)
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'tx/:txId/replaces',
				this.getRbfHistory,
			)
			.get(
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'donations',
				async (req, res) => {
					try {
						const response = await axios.get(
							`${config.EXTERNAL_DATA_SERVER.MEMPOOL_API}/donations`,
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'donations/images/:id',
				async (req, res) => {
					try {
						const response = await axios.get(
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'contributors',
				async (req, res) => {
					try {
						const response = await axios.get(
							`${config.EXTERNAL_DATA_SERVER.MEMPOOL_API}/contributors`,
							{ responseType: 'stream', timeout: 10000 },
						)
						response.data.pipe(res)
					} catch (e) {
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'contributors/images/:id',
				async (req, res) => {
					try {
						const response = await axios.get(
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'translators',
				async (req, res) => {
					try {
						const response = await axios.get(
							`${config.EXTERNAL_DATA_SERVER.MEMPOOL_API}/translators`,
							{ responseType: 'stream', timeout: 10000 },
						)
						response.data.pipe(res)
					} catch (e) {
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'translators/images/:id',
				async (req, res) => {
					try {
						const response = await axios.get(
			...
			.get(
				config.MEMPOOL.API_URL_PREFIX + 'blocks',
		...
		if (config.MEMPOOL.BACKEND !== 'esplora') {
			app.get(config.MEMPOOL.API_URL_PREFIX + 'mempool', this.getMempool)
				.get(
				...
				.get(
					config.MEMPOOL.API_URL_PREFIX + 'tx/:txId',
					this.getTransaction,
				)
				.post(
					config.MEMPOOL.API_URL_PREFIX + 'tx',
				...
				.get(
					config.MEMPOOL.API_URL_PREFIX +
						'address/:address/txs/chain/:txId',
					this.getAddressTransactions,
				)
				.get(
	...
	private async $getBatchedOutspends(req: Request, res: Response) {
		...
		try {
			const batchedOutspends = await bitcoinApi.$getBatchedOutspends(
				txIds,
			)
	...
	private async $getCpfpInfo(req: Request, res: Response) {
		...
		if (tx) {
			...
			if (config.DATABASE.ENABLED) {
				cpfpInfo = await transactionRepository.$getCpfpInfo(
					req.params.txId,
				)
			}
			if (cpfpInfo) {
	...
	private async getTransaction(req: Request, res: Response) {
		try {
			...
			if (
				...
				e instanceof Error &&
				e.message &&
				e.message.indexOf('No such mempool or blockchain transaction') >
					-1
			) {
				statusCode = 404
			}
	 ...
	 */
	private async postPsbtCompletion(
		req: Request,
		res: Response,
	): Promise<void> {
		res.setHeader('content-type', 'text/plain')
		...
		try {
			...
			for (const [index, input] of psbt.data.inputs.entries()) {
				if (!input.nonWitnessUtxo) {
					...
					try {
						transactionHex = await bitcoinApi.$getTransactionHex(
							txid,
						)
						if (!transactionHex) {
							throw new Error('')
			...
			if (
				e instanceof Error &&
				new RegExp(notFoundError).test(e.message)
			) {
	...
	private async getBlockHeader(req: Request, res: Response) {
		try {
			const blockHeader = await bitcoinApi.$getBlockHeader(
				req.params.hash,
			)
			res.setHeader('content-type', 'text/plain')
	...
	private async getBlockAuditSummary(req: Request, res: Response) {
		try {
			const transactions = await blocks.$getBlockAuditSummary(
				req.params.hash,
			)
			res.setHeader(
				'Expires',
	...
	private async getBlocks(req: Request, res: Response) {
		try {
			if (
				['mainnet', 'testnet', 'signet'].includes(
	...
	private async getBlocksByBulk(req: Request, res: Response) {
		try {
			if (
				...
				// Liquid, Bisq - Not implemented
				return res
					.status(404)
					.send(`This API is only available for Bitcoin networks`)
			}
			...
			if (!req.params.from || from < 0) {
				return res
					.status(400)
					.send(`Parameter 'from' must be a block height (integer)`)
			}
			const to =
			...
			if (to < 0) {
				return res
					.status(400)
					.send(`Parameter 'to' must be a block height (integer)`)
			}
			if (from > to) {
				return res
					.status(400)
	...
	private async getLegacyBlocks(req: Request, res: Response) {
		try {
			...
			const fromHeight = Math.min(
				parseInt(req.params.height, 10) || tip,
				tip,
			)
			...
			for (let i = 0; i < 10 && nextHash; i++) {
				const localBlock = blocks
					.getBlocks()
					.find(b => b.id === nextHash)
				if (localBlock) {
	...
	private async getBlockTransactions(req: Request, res: Response) {
		try {
			...
			const startingIndex = Math.max(
				0,
				parseInt(req.params.index || '0', 10),
			)
			...
			for (let i = startingIndex; i < endIndex; i++) {
				try {
					const transaction =
						await transactionUtils.$getTransactionExtended(
							txIds[i],
							true,
							true,
						)
					...
					loadingIndicators.setProgress(
						...
						((i - startingIndex + 1) / (endIndex - startingIndex)) *
							100,
					)
				} catch (e) {
					logger.debug(
	...
	private async getAddressPrefix(req: Request, res: Response) {
		try {
			const blockHash = await bitcoinApi.$getAddressPrefix(
				req.params.prefix,
			)
			res.send(blockHash)
		} catch (e) {
	...
	private async validateAddress(req: Request, res: Response) {
		try {
			const result = await bitcoinClient.validateAddress(
				req.params.address,
			)
			res.json(result)
		} catch (e) {
	...
	private printRoutes(layer, prefix = '') {
		if (layer.route) {
			layer.route.stack.forEach(this.printRoutes.bind(null, layer.route))
		} else if (layer.name === 'router' && layer.handle.stack) {
			layer.handle.stack.forEach(
				this.printRoutes.bind(
					null,
					layer.handle,
					prefix + layer.regexp,
				),
			...
			console.log(
				`${prefix} ${layer.method.toUpperCase()} -> ${layer.path}`,
			)
		}
	}
}
